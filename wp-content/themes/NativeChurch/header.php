...
<body <?php body_class($bodyClass);
    ...
    <div class="body header-style<?php echo esc_attr($header_layout); ?>">
        ...
        if (isset($options['header_layout']) && $options['header_layout'] == 3) :
            //-- Start Top Row --
            echo '
        <div class="toprow">
   			<div class="container">
        ...
        <header class="site-header">
            <div class="topbar">
                <div class="container hs4-cont">
                    <div class="row">
                        ...
                        <div class="col-md-4 col-sm-6 col-xs-8">
                            <h1 class="logo">
                                ...
                                if (isset($options['logo_alt_text']) && $options['logo_alt_text'] != "") { ?>
                                    <?php $logoalt = esc_html($options['logo_alt_text']);
                                } else {
                                    $logoalt = 'Logo';
                                } ?>
                                <?php
                                if (isset($options['logo_upload']) && !empty($options['logo_upload']['url'])) { ?>
                                    <a href="<?php echo esc_url(home_url()); ?>" class="default-logo" title="<?php echo esc_attr($logoalt); ?>"><img src="<?php echo esc_url($options['logo_upload']['url']); ?>" alt="<?php echo esc_attr($logoalt); ?>"></a>
                                <?php } else { ?>
                                ...
                                if (isset($options['retina_logo_upload']) && !empty($options['retina_logo_upload']['url'])) { ?>
                                    <a href="<?php echo esc_url(home_url()); ?>" title="<?php echo esc_attr($logoalt); ?>" class="retina-logo"><img src="<?php echo esc_url($options['retina_logo_upload']['url']); ?>" alt="<?php echo esc_attr($logoalt); ?>" width="<?php echo esc_attr($options['retina_logo_width']); ?>" height="<?php echo esc_attr($options['retina_logo_height']); ?>"></a>
                                <?php } elseif (isset($options['logo_upload']) && !empty($options['logo_upload']['url'])) { ?>
                                    <a href="<?php echo esc_url(home_url()); ?>" title="<?php echo esc_attr($logoalt); ?>" class="retina-logo"><img src="<?php echo esc_url($options['logo_upload']['url']); ?>" alt="<?php echo esc_attr($logoalt); ?>"></a>
                                <?php } else { ?>
                        ...
                        <?php
                        if (isset($options['header_layout']) && !empty($options['header_layout'])) :
                            echo '<div class="col-md-8 col-sm-6 col-xs-4 hs4-menu">';
                            if (isset($options['enable-top-menu']) && $options['enable-top-menu'] == 1) {
                            ...
                            if (isset($options['mobile_menu_text'])) {
            ...
            <?php
            $header_layout_menu = (isset($options['header_layout'])) ? $options['header_layout'] : '';
        ...
        if ((!is_front_page()) || $show_on_front == 'posts' || (!is_page_template('template-home.php') && !is_page_template('template-h-second.php') && !is_page_template('template-h-third.php') && !is_page_template('template-home-pb.php'))) {
            ...
            if (($header_options == 0 || $header_options == '') || (is_category() || !empty($term->term_id))) { ?>
                ...
                echo "</style>" . "\n"; ?>
                <div class="nav-backed-header parallax" style="background-image:url(<?php echo esc_url($src[0]); ?>);">
                    <div class="container">
                        <div class="row">
                            <div class="col-md-12">
                                <ol class="breadcrumb">
            ...
            <?php } elseif ($header_options == 3 || $header_options == '') { ?>
                ...
                echo '.body ol.breadcrumb{padding-top:' . $breadpad . 'px;}';
                echo "</style>" . "\n"; ?>
                <div class="nav-backed-header parallax" style="background-color:<?php echo esc_attr($color); ?>;">
                    <div class="container">
                        <div class="row">
                            <div class="col-md-12">
                                <ol class="breadcrumb">
                ...
                <div class="container">
                    <div class="row detail-page-title-bar">
                        ...
                        if (is_search()) {
                            ...
                            printf(esc_html__('Search Results for :  %s', 'framework'), get_search_query());
                            echo '</h1>
                                  </div>';
                        } else if (is_404()) {
                            echo '<div class="col-md-12 col-sm-12"><h1>';
                            esc_html_e('Error 404 - Not Found', 'framework');
                            echo '</h1>
                                  </div>';
                        } elseif (is_author()) {
                            global $author;
                            ...
                            echo '<div class="col-md-8 col-sm-8">
                                    <h1>' . esc_html__('All Posts For - ', 'framework') . $userdata->display_name . '</h1>
                                  </div>';
                            ...
                            if (!empty($event_cat)) {
                                echo '<h1>' . esc_html__('All Posts For ', 'framework') . strtoupper($event_cat) . '</h1>';
                            } else {
                                echo '<h1 class="cpt-page-title">' . $imic_post_custom_title . '</h1>';
                            }
                            echo '</div>';
                            if (is_page_template('template-gallery-filter.php')) {
                                ?>
                                    ...
                                    <ul class="nav nav-pills sort-source" data-sort-id="gallery" data-option-key="filter">
                                        ...
                                        <?php if (!empty($gallery_cats) && count($gallery_cats) > 1) {  ?>
                                            ...
                                            <?php foreach ($gallery_cats as $gallery_cat) { ?>
                                                <li data-option-value=".format-<?php echo esc_attr($gallery_cat); ?>">
                                                    <a href="#"><i class="fa <?php echo esc_attr($gallery_cat); ?>"></i>
                                                        <span><?php $term = get_term_by('slug', $gallery_cat, 'gallery-category');
                                                                $name = $term->name;
                                                                echo esc_html($name); ?></span></a></li>
                                            <?php }
                                        } ?>
                                        <?php if (empty($gallery_category)) { ?>
                                            ...
                                            <?php foreach ($gallery_cats_default as $gallery_cat_default) { ?>
                                                <li data-option-value=".format-<?php echo esc_attr($gallery_cat_default->slug); ?>"><a href="#"><i class="fa <?php echo esc_attr($gallery_cat_default->description); ?>"></i> <span><?php echo esc_attr($gallery_cat_default->name); ?></span></a></li>
                                            <?php } ?>
                                        <?php } ?>
                                    </ul>
                                </div>
                            ...
                            if (is_category() || is_tag()) {
                                echo '<div class="col-md-8 col-sm-8">
                                    <h1>' . esc_html__('All Posts For - ', 'framework') . single_term_title("", false) . '</h1>
                                  	</div>';
                            } else {
                                if ($page_for_posts == 0 && !is_single()) {
                                    $imic_post_custom_title = esc_html__('Blog', 'framework');
                                } else if ($page_for_posts > 0 && !is_single()) {
                                    $imic_post_custom_title = get_the_title($page_for_posts);
                                } else {
                                    $imic_post_custom_title = !empty($custom['imic_post_page_custom_title'][0]) ? $custom['imic_post_page_custom_title'][0] : esc_html__('Blog', 'framework');
                                }
                                echo '<div class="col-md-8 col-sm-8">
                                    <h1 class="cpt-page-title">' . $imic_post_custom_title . '</h1>
                                  	</div>';
                                if (!empty($custom['imic_post_custom_description'][0])) {
                                    echo '<div class="col-md-4 col-sm-4">
                                        <p class="custom-desc">' . $custom['imic_post_custom_description'][0] . '</p>
                                     	</div>';
                                }
                            }
                            ...
                            if (!empty($term->term_id)) {
                                echo '<div class="col-md-12">
                                   	<h1>' . esc_html__('All Sermons For ', 'framework') . $term->name . '</h1>
                                   	</div>';
                            } else {
                                $imic_post_custom_title = !empty($custom['imic_post_page_custom_title'][0]) ? $custom['imic_post_page_custom_title'][0] : esc_html__('Sermons', 'framework');
                            ...
                            if (!empty($term->term_id)) {
                                echo '<div class="col-md-12">
                                        <h1>' . esc_html__('All Causes For - ', 'framework') . $term->name . '</h1>
                                      </div>';
                            } else {
                                ...
                                if (!empty($sterm)) {
                                    ...
                                    foreach ($sterm as $terms) {
                                        if ($i == 1) {
                                            ...
                                            echo '<div class="col-md-2 col-sm-2 col-xs-4">
                                                <a href="' . $term_link . '" class="pull-right btn btn-primary">' . esc_html__('All causes', 'framework') . '</a>
                                                </div>';
                                        }
                                        $i++;
                                    }
                            ...
                            if (!empty($term->term_id)) {
                                echo '<div class="col-md-12">
                                        <h1>' . esc_html__('All Staff For - ', 'framework') . $term->name . '</h1>
                                      </div>';
                            } else {
                                $imic_post_custom_title = !empty($custom['imic_post_page_custom_title'][0]) ? $custom['imic_post_page_custom_title'][0] : esc_html__('Team', 'framework');
                                ...
                                if (!empty($sterm)) {
                                    ...
                                    foreach ($sterm as $terms) {
                                        if ($i == 1) {
                                            ...
                                            echo '<div class="col-md-2 col-sm-2 col-xs-4">
                                                <a href="' . $term_link . '" class="pull-right btn btn-primary">' . esc_html__('All staff', 'framework') . '</a>
                                                </div>';
                                        }
                                        $i++;
                                    }
                            ...
                            if (!empty($term->term_id)) {
                                echo '<div class="col-md-12">
                                        <h1>' . esc_html__('All Events For - ', 'framework') . $term->name . '</h1>
                             
                                      </div>';
                            } else {
                                $imic_post_custom_title = !empty($custom['imic_post_page_custom_title'][0]) ? $custom['imic_post_page_custom_title'][0] : esc_html__('Events', 'framework');
                            ...
                            if (!empty($eterm)) {
                                ...
                                foreach ($eterm as $terms) {
                                    if ($i == 1) {
                                        ...
                                        echo '<div class="col-md-2 col-sm-2 col-xs-4">
                                     <a href="' . $term_link . '" class="pull-right btn btn-primary">' . esc_html__('All events', 'framework') . '</a></div>';
                                    }
                                    $i++;
                                }
                            }
                            ...
                            if (!empty($term->term_id)) {
                                echo '<div class="col-md-12">
                                        <h1>' . esc_html__('All Products For - ', 'framework') . $term->name . '</h1>
                                      </div>';
                            } else {
                                $variable_post_id = '';
                                if (is_single()) :
